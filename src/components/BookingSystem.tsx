...
export function BookingSystem() {
  ...
  const [reunionTime, setReunionTime] = useState<string>("");
  ...
  // Update Reunion time every minute
  useEffect(() => {
    const updateReunionTime = () => {
      const now = new Date();
      const reunionNow = new Date(now.getTime() + (4 * 60 * 60 * 1000)); // UTC+4
      setReunionTime(reunionNow.toLocaleTimeString('fr-FR', { 
        hour: '2-digit', 
        minute: '2-digit',
        timeZone: 'UTC'
      }));
    ...
    updateReunionTime();
    const interval = setInterval(updateReunionTime, 60000); // Update every minute
  ...
  useEffect(() => {
    if (selectedDate) {
      ...
      // Réinitialiser l'état de soumission lors du changement de date
      setSubmitted(false);
    }
  }, [selectedDate, fetchAvailableSlots]);

  // Réinitialiser l'état de soumission lors du changement d'heure
  useEffect(() => {
    if (selectedTime && submitted) {
      setSubmitted(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // La configuration EmailJS est maintenant intégrée, pas besoin de vérification
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Envoyer les emails de confirmation côté client
        try {
          console.log('📧 Envoi des emails de confirmation côté client...');
        ...
        // Reset form après un délai pour laisser le temps à l'utilisateur de voir le message
        setTimeout(() => {
          setFormData({
            name: "",
            email: "",
      ...
      <div className="container mx-auto px-4">
        <div className="text-center space-y-4 mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto text-center">
            Donnons vie à votre vision créative ! Un échange de 20 minutes pour explorer vos possibilités.
          </p>
        </div>
                      ...
                      <div className="text-center py-8 bg-muted/50 rounded-lg">
                        ...
                        {selectedDate === new Date().toISOString().split('T')[0] && (
                          <p className="text-xs text-muted-foreground">
                            ⚠️ Les réservations pour aujourd'hui ne sont plus possibles
                          </p>
                        )}
                      </div>
                    )}
